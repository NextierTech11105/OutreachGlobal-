...
export class SubscriptionService implements OnModuleInit {
  ...
  private async ensureStarterPlanExists() {
    try {
      ...
      if (!existing) {
        // Generate a candidate id using the same prefix/ULID pattern used elsewhere
        const candidateId = `plan_${ulid()}`;

        // Check plans.id column max length to avoid "value too long" DB errors
        let planIdMaxLen: number | null = null;
        try {
          const colInfo = await this.db.execute(
            sql`SELECT character_maximum_length FROM information_schema.columns WHERE table_name = 'plans' AND column_name = 'id' LIMIT 1`,
          );
          const rawMaxLen = colInfo.rows?.[0]?.character_maximum_length;
          planIdMaxLen = Number.isFinite(Number(rawMaxLen))
            ? Number(rawMaxLen)
            : null;
        } catch (colErr) {
          this.logger.warn(
            "Could not read plans.id column metadata; proceeding with insert and relying on DB constraints.",
            (colErr as any)?.message || colErr,
          );
        }

        if (planIdMaxLen && candidateId.length > planIdMaxLen) {
          // Non-fatal: log and skip creating the starter plan to avoid blocking startup
          this.logger.warn(
            `Starter plan id (${candidateId}) length ${candidateId.length} exceeds plans.id max length ${planIdMaxLen}; skipping starter plan insert. Apply migration to widen this column.`,
          );
        } else {
          ...
          try {
            ...
            await this.db
              ...
              .values({
                ...
                isActive: true,
                sortOrder: 1,
                // ensure timestamps are Date objects to avoid driver mapping errors
                createdAt: new Date(),
                updatedAt: new Date(),
              })
              .onConflictDoNothing();
            this.logger.log("Created starter plan (or already exists)");
              ...
              // Log and continue; seeding failures should not block startup
              this.logger.warn(
                "Starter plan insert failed (non-fatal). Please inspect DB column sizes and constraints.",
      ...
      try {
        ...
        this.logger.log(
          `plans table columns: ${JSON.stringify(cols.rows || cols)}`,
        );
      ...
      // Non-fatal: Do not re-throw. Seeding failures should not block application startup.
      this.logger.warn(
        "Starter plan seed failed but will not block startup. Please investigate the error above.",
      );
      return;
    }
  }
