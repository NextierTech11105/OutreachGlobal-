...
export class OwnerRecoveryController {
  ...
  async restoreOwner(@Req() req: FastifyRequest) {
    const enabled =
      this.configService.get<string>("ENABLE_EMERGENCY_ADMIN") === "true";
    if (!enabled) {
    ...
    const provided = (
      req.headers["x-owner-recovery-token"] as string | undefined
    )?.trim();
    if (
      !expected ||
      !provided ||
    ...
    const allowlist = (
      this.configService.get<string>("OWNER_RECOVERY_IP_ALLOWLIST") ||
      "127.0.0.1,::1"
    )
      .split(",")
      .map((v) => v.trim())
    ...
    const context =
      await this.ownerRecoveryService.ensureOwnerAnchor("emergency-route");
    this.logger.log(
      `Emergency restore invoked at ${new Date().toISOString()} from ${reqIp}`,
    );
