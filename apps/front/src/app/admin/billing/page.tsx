...
export default function BillingDashboard() {
    ...
    try {
      ...
      const [plansRes, usageRes] = await Promise.all([
        fetch("/api/billing/plans")
          .then((r) => r.json())
          .catch(() => ({ plans: [] })),
        fetch("/api/billing/usage?userId=admin&history=true")
          .then((r) => r.json())
          .catch(() => null),
      ]);
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      ...
      case "past_due":
        return (
          <Badge className="bg-yellow-500/20 text-yellow-500">Past Due</Badge>
        );
      case "canceled":
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent>
              ...
              <div className="flex items-center mt-2">
                ...
                <span
                  className={`text-xs ${stats.mrrChange >= 0 ? "text-green-500" : "text-red-500"}`}
                >
                  {stats.mrrChange >= 0 ? "+" : ""}
                  {stats.mrrChange}%
                </span>
            ...
            <CardContent>
              ...
              <p className="text-xs text-zinc-500 mt-2">
                {stats.activeSubscriptions === 0
                  ...
                  : `+${stats.subscriptionChange} this month`}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent>
              ...
              <p className="text-xs text-zinc-500 mt-2">Monthly churn rate</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-6">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span>
                    {sf(usageOverview.leadsUsed)} /{" "}
                    {sf(usageOverview.leadsLimit)}
                  </span>
                </div>
                <Progress
                  value={
                    (usageOverview.leadsUsed / usageOverview.leadsLimit) * 100
                  }
                  className="h-2"
                />
              </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span>
                    {sf(usageOverview.smsUsed)} / {sf(usageOverview.smsLimit)}
                  </span>
                </div>
                <Progress
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span>
                    {sf(usageOverview.skipTracesUsed)} /{" "}
                    {sf(usageOverview.skipTracesLimit)}
                  </span>
                </div>
                <Progress
                  value={
                    (usageOverview.skipTracesUsed /
                      usageOverview.skipTracesLimit) *
                    100
                  }
                  className="h-2"
                />
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span>
                    {sf(usageOverview.searchesUsed)} /{" "}
                    {sf(usageOverview.searchesLimit)}
                  </span>
                </div>
                <Progress
                  value={
                    (usageOverview.searchesUsed / usageOverview.searchesLimit) *
                    100
                  }
                  className="h-2"
                />
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-400 mb-3">
                  To enable payment processing, configure your Stripe API keys
