...
const AGENT_INFO = {
  datalake: {
    name: "Datalake",
    tagline: "DATA SOURCE",
    description: "Query your uploaded business lists. Search by keyword, sector, or location.",
    color: "cyan",
    icon: Database,
    step: 1,
  },
  luci: {
    ...
    description: "Search keywords in lists, find decision makers, skip trace, push to campaigns.",
    color: "red",
    icon: Globe,
    step: 2,
  },
  gianna: {
    ...
    description: "Manages inbound messages for all campaign numbers. Auto-replies and books calls.",
    color: "pink",
    icon: MessageSquare,
    step: 3,
  },
            ...
            <div className="flex items-center justify-between">
              {Object.entries(AGENT_INFO).map(([key, info], index) => {
                ...
                const colorClasses: Record<string, string> = {
                  cyan: isActive ? "text-cyan-400 border-cyan-400" : "text-zinc-500 border-zinc-700",
                  red: isActive ? "text-red-400 border-red-400" : "text-zinc-500 border-zinc-700",
                  pink: isActive ? "text-pink-400 border-pink-400" : "text-zinc-500 border-zinc-700",
                  amber: isActive ? "text-amber-400 border-amber-400" : "text-zinc-500 border-zinc-700",
                };
                ...
                return (
                  <div key={key} className="flex items-center">
                    ...
                    >
                      <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold ${colorClasses[info.color]}`}>
                        {info.step}
                      </span>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  {(() => {
                    ...
                    return (
                      <>
                        <div className={`p-3 rounded-xl border ${bgColors[info.color]}`}>
                          <Icon className={`h-6 w-6 ${textColors[info.color]}`} />
                        </div>
                        <div>
                          <div className="flex items-center gap-3">
                            <Badge variant="outline" className={`text-xs font-bold ${textColors[info.color]} border-current`}>
                              STEP {info.step}
                            </Badge>
                            <h2 className="text-xl font-black">{info.name}</h2>
                            <Badge variant="outline" className="text-xs font-medium text-zinc-400">
                              {info.tagline}
                            </Badge>
                          </div>
