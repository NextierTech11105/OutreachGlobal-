...
function getNeighborhoodData(city?: string, state?: string) {
  const data: Record<string, { facts: string[]; timeline: Array<{ year: string; event: string }> }> = {
    "highland_ny": {
      facts: [
      ...
      timeline: [
        { year: "1900s", event: "Agricultural era - farms and orchards dominate the landscape" },
        { year: "1930s", event: "Mid-Hudson Bridge opens, connecting to Poughkeepsie" },
        { year: "1970s", event: "Suburban expansion brings new development" },
        { year: "2000s", event: "Hudson Valley renaissance - wineries and tourism flourish" },
        { year: "2020s", event: "Remote work migration drives record home values" },
      ],
    },
    "brooklyn_ny": {
      facts: [
      ...
      timeline: [
        { year: "1800s", event: "Dutch settlement 'Breuckelen' becomes a city" },
    ...
    "default": {
      ...
      timeline: [
        ...
        { year: "1950s", event: "Post-war growth and infrastructure improvements" },
...
function getNeighborhoodEnrichment(city?: string, state?: string) {
  ...
  const defaultData = {
    employers: [
      { name: "Regional Medical Center", type: "Healthcare", employees: "2,500+", icon: "hospital" },
      { name: "County School District", type: "Education", employees: "1,800+", icon: "school" },
      { name: "Manufacturing Corp", type: "Manufacturing", employees: "1,200+", icon: "factory" },
      { name: "Tech Solutions Inc", type: "Technology", employees: "800+", icon: "office" },
    ],
    transportation: [
    ...
    businesses: [
      { name: "Whole Foods Market", type: "Grocery", distance: "0.8 mi", icon: "shopping" },
      { name: "Main Street Cafe", type: "Restaurant", distance: "0.4 mi", icon: "restaurant" },
      { name: "Town Square Mall", type: "Shopping", distance: "2.1 mi", icon: "store" },
      { name: "Community Fitness", type: "Gym", distance: "1.2 mi", icon: "fitness" },
      { name: "Starbucks", type: "Coffee", distance: "0.6 mi", icon: "coffee" },
      { name: "CVS Pharmacy", type: "Pharmacy", distance: "0.5 mi", icon: "pharmacy" },
    ],
  };
  ...
  const cityData: Record<string, typeof defaultData> = {
    "highland_ny": {
      employers: [
        { name: "Vassar Brothers Medical", type: "Healthcare", employees: "3,200+", icon: "hospital" },
        { name: "IBM Hudson Valley", type: "Technology", employees: "2,800+", icon: "office" },
        { name: "SUNY New Paltz", type: "Education", employees: "1,500+", icon: "school" },
        { name: "Central Hudson", type: "Utilities", employees: "1,100+", icon: "factory" },
      ],
      transportation: [
      ...
      businesses: [
        { name: "Adams Fairacre Farms", type: "Grocery", distance: "1.2 mi", icon: "shopping" },
        { name: "Highland Landing", type: "Restaurant", distance: "0.8 mi", icon: "restaurant" },
        { name: "Poughkeepsie Galleria", type: "Shopping", distance: "9 mi", icon: "store" },
        { name: "Walkway Over Hudson", type: "Recreation", distance: "3 mi", icon: "park" },
        { name: "Local Wineries", type: "Entertainment", distance: "5 mi", icon: "wine" },
        { name: "Rite Aid", type: "Pharmacy", distance: "0.4 mi", icon: "pharmacy" },
      ],
    },
    "brooklyn_ny": {
      employers: [
        { name: "NYC Health + Hospitals", type: "Healthcare", employees: "12,000+", icon: "hospital" },
        { name: "Brooklyn Navy Yard", type: "Mixed Use", employees: "10,000+", icon: "factory" },
        { name: "CUNY Brooklyn College", type: "Education", employees: "3,500+", icon: "school" },
        { name: "Industry City", type: "Tech/Creative", employees: "8,000+", icon: "office" },
      ],
      transportation: [
      ...
      businesses: [
        { name: "Whole Foods / Trader Joe's", type: "Grocery", distance: "0.5 mi", icon: "shopping" },
        { name: "Hundreds of Restaurants", type: "Dining", distance: "0.1 mi", icon: "restaurant" },
        { name: "Atlantic Terminal Mall", type: "Shopping", distance: "2 mi", icon: "store" },
        { name: "Brooklyn Boulders", type: "Fitness", distance: "1 mi", icon: "fitness" },
        { name: "Artisanal Coffee Shops", type: "Coffee", distance: "0.1 mi", icon: "coffee" },
        { name: "24hr Pharmacies", type: "Pharmacy", distance: "0.3 mi", icon: "pharmacy" },
      ],
    },
  };
...
function getHistoricalContext(yearBuilt?: number): string {
  ...
  const contexts: Record<string, string> = {
    pre1900: "Built during the Gilded Age, a time of rapid economic growth and architectural innovation",
    "1900s": "Constructed in the early 20th century, reflecting the craftsmanship of its era",
    "1920s": "Built during the Roaring Twenties, a decade of prosperity and cultural dynamism",
    "1950s": "A product of the 1950s suburban expansion era with mid-century design",
...
export function ReportView({ reportId }: ReportViewProps) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 flex items-center justify-center p-8">
        <div className="text-center max-w-sm">
          ...
          <p className="text-slate-400">{error || "This report may have expired."}</p>
        </div>
      </div>
    );
  }

  const { property, valuation, comparables, neighborhood, demographics, aiAnalysis, partnerOffer, leadInfo, companyInfo, agentInfo } = report;
  const address = property?.address;
  const estimatedValue = valuation?.estimatedValue || property?.estimatedValue || 0;
  const neighborhoodData = getNeighborhoodData(address?.city, address?.state);
  const neighborhoodEnrichment = getNeighborhoodEnrichment(address?.city, address?.state);
  ...
  const compScatterData = comparables?.map((c, i) => ({
    sqft: c.squareFeet || 0,
    price: c.salePrice || 0,
    address: c.address,
    isSubject: false,
        ...
        <div className="relative max-w-4xl mx-auto px-6 pt-8 pb-10">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-bold text-white text-xl block">{company.name}</span>
                {company.tagline && (
                  <span className="text-slate-400 text-xs">{company.tagline}</span>
                )}
              </div>
            </div>
            <div className="text-right">
              {leadInfo?.name && (
                <p className="text-slate-400 text-sm">Prepared for <span className="text-white font-medium">{leadInfo.name}</span></p>
              )}
              {company.phone && (
          ...
          <div className="text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 mb-6">
              {estimatedValue > 0 && (
                <div className="px-4 py-2 bg-emerald-500/20 rounded-full border border-emerald-500/30">
                  <span className="text-emerald-400 font-bold text-lg">{formatCurrency(estimatedValue)}</span>
                  <span className="text-emerald-300/60 text-sm ml-1">Est. Value</span>
                </div>
              )}
              {property?.squareFeet && (
            ...
            <p className="text-slate-500 text-sm max-w-lg mx-auto">
              Discover the story behind this property — from neighborhood character to investment potential
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <section className="bg-gradient-to-br from-slate-800 to-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">{address?.city || "Your Neighborhood"}</h2>
                  <p className="text-slate-400">Community Character & History</p>
                </div>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="space-y-3">
                  {neighborhoodData.facts.map((fact, i) => (
                    <div key={i} className="flex items-start gap-4 p-4 bg-slate-900/50 rounded-xl border border-slate-700/30">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {neighborhood?.schoolRating && (
                  <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-3xl font-bold text-white">{neighborhood.schoolRating}<span className="text-lg text-slate-500">/10</span></p>
                ...
                {neighborhood?.walkScore && (
                  <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-3xl font-bold text-white">{neighborhood.walkScore}</p>
                ...
                {neighborhood?.population && (
                  <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-3xl font-bold text-white">{(neighborhood.population / 1000).toFixed(0)}K</p>
                    <p className="text-xs text-slate-500 mt-1">Population</p>
                  </div>
                )}
                <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                  <TrendingUp className="h-6 w-6 text-emerald-400 mx-auto mb-2" />
                  <p className="text-3xl font-bold text-white">+{neighborhood?.appreciation || 8}%</p>
                  ...
                  {neighborhoodData.timeline.map((item, i) => (
                    <div key={i} className="relative pl-12">
                      ...
                      <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-700/30">
                        <p className="text-blue-400 font-bold text-lg">{item.year}</p>
            ...
            {/* Appreciation Chart */}
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              <h3 className="text-xl font-bold text-white mb-6">Home Value Trends (Area Average)</h3>
              <div className="h-64">
            ...
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              <p className="text-slate-400 text-sm mb-6">
                Understanding who lives here helps paint the full picture of this neighborhood.
              </p>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                  <p className="text-3xl font-bold text-white">{demographics?.medianAge || neighborhood?.population ? Math.round(38 + Math.random() * 8) : 38}</p>
                  <p className="text-xs text-slate-500 mt-1">Median Age</p>
                </div>
                <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                  <p className="text-3xl font-bold text-white">{demographics?.medianIncome ? formatCurrency(demographics.medianIncome) : neighborhood?.medianIncome ? formatCurrency(neighborhood.medianIncome) : "$75K"}</p>
                  <p className="text-xs text-slate-500 mt-1">Median Income</p>
                </div>
                <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                  <p className="text-3xl font-bold text-white">{demographics?.ownerOccupied || 65}%</p>
                  <p className="text-xs text-slate-500 mt-1">Owner Occupied</p>
                </div>
                <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                  <p className="text-3xl font-bold text-white">{demographics?.educationBachelors || 32}%</p>
                  <p className="text-xs text-slate-500 mt-1">College Educated</p>
                </div>
              </div>

              {/* Ownership vs Rental breakdown */}
              <div className="bg-slate-900/40 rounded-2xl p-6">
                <h4 className="text-white font-semibold mb-4">Housing Tenure</h4>
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-slate-400 text-sm">Owner-Occupied</span>
                      <span className="text-blue-400 font-semibold">{demographics?.ownerOccupied || 65}%</span>
                    </div>
                    <div className="h-3 bg-slate-700 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-blue-500 to-blue-400 rounded-full"
                        style={{ width: `${demographics?.ownerOccupied || 65}%` }}
                      />
                    </div>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-slate-400 text-sm">Renter-Occupied</span>
                      <span className="text-amber-400 font-semibold">{demographics?.renterOccupied || 35}%</span>
                    </div>
                    <div className="h-3 bg-slate-700 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-amber-500 to-amber-400 rounded-full"
                        style={{ width: `${demographics?.renterOccupied || 35}%` }}
                      />
                    </div>
                  </div>
                </div>
                {demographics?.medianRent && (
                  <p className="text-slate-500 text-sm mt-4">
                    Average rent in this area: {formatCurrency(demographics.medianRent)}/month
                  </p>
                )}
              </div>
            ...
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {neighborhoodEnrichment.employers.map((employer, i) => (
                  <div key={i} className="bg-slate-900/40 rounded-2xl p-5 flex items-start gap-4 border border-slate-700/30 hover:border-amber-500/30 transition-colors">
                    <div className="w-12 h-12 bg-amber-500/10 rounded-xl flex items-center justify-center flex-shrink-0">
                      {employer.icon === "hospital" && <Stethoscope className="h-6 w-6 text-amber-400" />}
                      {employer.icon === "school" && <GraduationCap className="h-6 w-6 text-amber-400" />}
                      {employer.icon === "factory" && <Factory className="h-6 w-6 text-amber-400" />}
                      {employer.icon === "office" && <Building2 className="h-6 w-6 text-amber-400" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-semibold truncate">{employer.name}</p>
                      <p className="text-slate-500 text-sm">{employer.type}</p>
                      <p className="text-amber-400 text-sm font-medium mt-1">{employer.employees} employees</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {neighborhoodEnrichment.transportation.map((item, i) => (
                  <div key={i} className="bg-slate-900/40 rounded-2xl p-5 flex items-center gap-4 border border-slate-700/30 hover:border-cyan-500/30 transition-colors">
                    <div className="w-12 h-12 bg-cyan-500/10 rounded-xl flex items-center justify-center flex-shrink-0">
                      {item.type === "train" && <Train className="h-6 w-6 text-cyan-400" />}
                      {item.type === "highway" && <Car className="h-6 w-6 text-cyan-400" />}
                      {item.type === "airport" && <Building className="h-6 w-6 text-cyan-400" />}
                      {item.type === "bus" && <Car className="h-6 w-6 text-cyan-400" />}
                      {item.type === "ferry" && <Car className="h-6 w-6 text-cyan-400" />}
                    </div>
                    <div className="flex-1">
            ...
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {neighborhoodEnrichment.businesses.map((biz, i) => (
                  <div key={i} className="bg-slate-900/40 rounded-xl p-4 flex items-center gap-3 border border-slate-700/30 hover:border-pink-500/30 transition-colors">
                    <div className="w-10 h-10 bg-pink-500/10 rounded-lg flex items-center justify-center flex-shrink-0">
                      {biz.icon === "shopping" && <ShoppingBag className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "restaurant" && <Utensils className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "store" && <Store className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "coffee" && <Coffee className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "pharmacy" && <Stethoscope className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "fitness" && <Users className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "park" && <TreePine className="h-5 w-5 text-pink-400" />}
                      {biz.icon === "wine" && <Coffee className="h-5 w-5 text-pink-400" />}
                    </div>
                    <div className="flex-1 min-w-0">
            ...
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              {historicalContext && property?.yearBuilt && (
                <div className="p-6 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-2xl border border-blue-500/20 mb-8">
                  <p className="text-slate-300 leading-relaxed text-lg">
                    <span className="text-blue-400 font-bold">Built in {property.yearBuilt}:</span> {historicalContext}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                {property?.yearBuilt && (
                  <div className="bg-slate-900/40 rounded-2xl p-5 text-center">
                    ...
                    <p className="text-4xl font-bold text-white">{property.yearBuilt}</p>
              ...
              {property?.propertyType && (
                <div className="bg-slate-900/40 rounded-xl p-4 flex items-center justify-between">
                  ...
                  <span className="text-white font-semibold">{property.propertyType}</span>
                </div>
              )}
            </section>
          </div>
        ...
        {activeTab === "valuation" && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <section className="relative overflow-hidden bg-gradient-to-br from-emerald-600 to-teal-600 rounded-3xl p-10 text-center">
              ...
              <div className="relative">
                <p className="text-emerald-100 text-sm font-medium mb-2">Estimated Market Value</p>
                ...
                {valuation?.valueRangeLow && valuation?.valueRangeHigh && (
                  <p className="text-emerald-100/80">
                    Range: {formatCurrency(valuation.valueRangeLow)} – {formatCurrency(valuation.valueRangeHigh)}
                  </p>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-slate-400 text-sm uppercase tracking-wider">Price Per Sq Ft</span>
                  <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded-lg text-xs font-semibold">FAVORABLE</span>
                </div>
                <p className="text-4xl font-bold text-white">${pricePerSqFt}</p>
                <p className="text-slate-500 text-sm mt-2">Below area average, indicating value opportunity</p>
              </div>

              <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-slate-400 text-sm uppercase tracking-wider">5-Year Projection</span>
                  <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded-lg text-xs font-semibold">+25%</span>
                </div>
                <p className="text-4xl font-bold text-white">{formatCurrency(estimatedValue * 1.25)}</p>
                <p className="text-slate-500 text-sm mt-2">Based on historical appreciation trends</p>
              </div>
            </div>

            {/* Projection Chart */}
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              <h3 className="text-xl font-bold text-white mb-6">5-Year Value Projection</h3>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={projectionData}>
                    ...
                    />
                    <Line type="monotone" dataKey="conservative" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 5" dot={false} name="Conservative (+3%)" />
                    <Line type="monotone" dataKey="base" stroke="#3b82f6" strokeWidth={3} dot={{ r: 4 }} name="Base Case (+5%)" />
                    <Line type="monotone" dataKey="optimistic" stroke="#10b981" strokeWidth={2} strokeDasharray="5 5" dot={false} name="Optimistic (+7%)" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              <div className="flex justify-center gap-6 mt-4 text-sm">
                <span className="flex items-center gap-2"><span className="w-3 h-0.5 bg-amber-500" />Conservative +3%</span>
                <span className="flex items-center gap-2"><span className="w-3 h-1 bg-blue-500" />Base +5%</span>
                <span className="flex items-center gap-2"><span className="w-3 h-0.5 bg-emerald-500" />Optimistic +7%</span>
              </div>
            </section>
              ...
              <section className="bg-gradient-to-br from-purple-900/30 to-blue-900/30 rounded-3xl p-8 border border-purple-500/20">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">Intelligent insights</p>
                  </div>
                </div>
                <p className="text-slate-300 leading-relaxed text-lg mb-6">{aiAnalysis.summary}</p>

                {aiAnalysis.strengths && aiAnalysis.strengths.length > 0 && (
                  <div className="space-y-3">
                    <p className="text-emerald-400 text-sm font-semibold">Key Strengths</p>
                    {aiAnalysis.strengths.map((s, i) => (
                      <div key={i} className="flex items-start gap-3 bg-emerald-500/10 rounded-xl p-3 border border-emerald-500/20">
        ...
        {activeTab === "market" && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              <p className="text-slate-400 mb-6">Recent sales of similar properties within 2 miles</p>

              <div className="space-y-3 mb-8">
                {comparables?.slice(0, 5).map((comp, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-slate-900/40 rounded-2xl hover:bg-slate-900/60 transition-colors">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-white truncate">{comp.address}</p>
                    ...
                    <div className="text-right ml-4">
                      <p className="font-bold text-emerald-400 text-lg">{formatCurrency(comp.salePrice)}</p>
                      {comp.squareFeet && comp.salePrice && (
                        <p className="text-xs text-slate-500">${Math.round(comp.salePrice / comp.squareFeet)}/sqft</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              {/* Scatter Chart */}
              <div className="bg-slate-900/40 rounded-2xl p-6">
                <h4 className="text-white font-semibold mb-4">Price vs. Square Footage</h4>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart>
                      <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                ...
                <div className="flex justify-center gap-6 mt-4 text-sm">
                  <span className="flex items-center gap-2"><span className="w-3 h-3 bg-blue-500 rounded-full" />Comparables</span>
                  <span className="flex items-center gap-2"><span className="w-3 h-3 bg-red-500 rounded-full" />Your Property</span>
                </div>
              </div>
            </section>
          </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <section className="bg-gradient-to-br from-slate-800 to-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Value-Add Analysis</h2>
                  <p className="text-slate-400">Contextual strategies to maximize your position</p>
                </div>
              </div>
              <p className="text-slate-300 leading-relaxed">
                Every property has untapped potential. Based on this property's characteristics, location, and market conditions,
                here are the most relevant strategies to consider — whether you're looking to build equity, generate income, or find the best exit.
              </p>
            </section>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6 border-l-4 border-blue-500">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-blue-500/20 text-blue-400 text-xs font-semibold rounded-full">HIGH IMPACT</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-slate-900/60 rounded-xl">
                    ...
                    <div>
                      <p className="text-slate-500 text-xs">Potential Value Add</p>
                      <p className="text-emerald-400 font-bold">+{formatCurrency(estimatedValue * 0.25)} - {formatCurrency(estimatedValue * 0.35)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6 border-l-4 border-purple-500">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-purple-500/20 text-purple-400 text-xs font-semibold rounded-full">PROVEN ROI</span>
                  </div>
                  <p className="text-slate-400 text-sm mb-4">
                  ...
                  <div className="space-y-3">
                    {[
                      { name: "Kitchen Remodel", cost: "$25K-60K", roi: "75-100%", impact: "Highest buyer appeal" },
                      { name: "Bathroom Update", cost: "$10K-25K", roi: "70-80%", impact: "Strong ROI" },
                      { name: "Curb Appeal", cost: "$5K-15K", roi: "100%+", impact: "First impression" },
                    ].map((reno, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-slate-900/60 rounded-xl">
                        <div>
                          <p className="text-white text-sm font-medium">{reno.name}</p>
                          <p className="text-slate-500 text-xs">{reno.impact}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-slate-400 text-xs">{reno.cost}</p>
                          <p className="text-emerald-400 text-sm font-semibold">{reno.roi} ROI</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6 border-l-4 border-emerald-500">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-emerald-500/20 text-emerald-400 text-xs font-semibold rounded-full">PASSIVE INCOME</span>
                  </div>
                  <p className="text-slate-400 text-sm mb-4">
                    Based on local market rents and property characteristics, here's the rental income potential:
                  </p>
                  <div className="grid grid-cols-3 gap-4 p-4 bg-slate-900/60 rounded-xl">
                    <div>
                      <p className="text-slate-500 text-xs">Est. Monthly Rent</p>
                      <p className="text-white font-bold">{formatCurrency(Math.round(estimatedValue * 0.006))}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">Annual Gross</p>
                      <p className="text-white font-bold">{formatCurrency(Math.round(estimatedValue * 0.006 * 12))}</p>
                    </div>
                    <div>
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6 border-l-4 border-amber-500">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-amber-500/20 text-amber-400 text-xs font-semibold rounded-full">HIGHER YIELD</span>
                  </div>
                  <p className="text-slate-400 text-sm mb-4">
                  ...
                  <div className="flex items-center gap-2 p-3 bg-amber-500/10 rounded-xl border border-amber-500/20">
                    ...
                    <p className="text-amber-200 text-sm">Verify local short-term rental regulations before proceeding.</p>
                  </div>
                </div>
                ...
                {property?.bedrooms && property.bedrooms >= 3 && (
                  <div className="bg-slate-900/40 rounded-2xl p-6 border-l-4 border-cyan-500">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="px-3 py-1 bg-cyan-500/20 text-cyan-400 text-xs font-semibold rounded-full">LIVE FREE</span>
                    </div>
                    <p className="text-slate-400 text-sm">
                      With {property.bedrooms} bedrooms, you could rent out spare rooms to offset your mortgage.
                      A single room could bring in $800-1,500/month depending on location.
                    </p>
                  </div>
                )}
              </div>
            ...
            <section className="bg-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
              ...
              <p className="text-slate-400 text-sm mb-6">
                Different situations call for different exits. Here's what makes sense based on your equity position:
              </p>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-blue-500/20 text-blue-400 text-xs font-semibold rounded-full">MAX VALUE</span>
                  </div>
                  <p className="text-slate-400 text-sm mb-3">
                    Best if: You have equity, can wait 3-6 months, and want top dollar.
                  </p>
                  <div className="flex items-center gap-4 text-sm">
                    <span className="text-slate-500">Timeline: 90-180 days</span>
                    <span className="text-slate-500">|</span>
                    <span className="text-slate-500">Costs: 6-10% of sale price</span>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-amber-500/20 text-amber-400 text-xs font-semibold rounded-full">FAST EXIT</span>
                  </div>
                  <p className="text-slate-400 text-sm mb-3">
                    Best if: Property needs work, you want quick cash, or there's little equity to maximize.
                  </p>
                  <div className="flex items-center gap-4 text-sm">
                    <span className="text-slate-500">Timeline: 7-30 days</span>
                    <span className="text-slate-500">|</span>
                    <span className="text-slate-500">Expect: 65-75% of ARV</span>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/40 rounded-2xl p-6">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="px-3 py-1 bg-violet-500/20 text-violet-400 text-xs font-semibold rounded-full">LOW/NO EQUITY</span>
                  </div>
                  <p className="text-slate-400 text-sm mb-3">
                    Best if: Little to no equity, behind on payments, or underwater. Options include subject-to, lease-option, or short sale.
                  </p>
                  <div className="flex items-center gap-4 text-sm">
                    <span className="text-slate-500">Timeline: Varies</span>
                    <span className="text-slate-500">|</span>
                    <span className="text-slate-500">Goal: Exit clean, protect credit</span>
                  </div>
                </div>
              </div>
            </section>
        ...
        {partnerOffer && (
          <section className="mt-8 relative overflow-hidden bg-gradient-to-r from-violet-600 to-purple-600 rounded-3xl p-8">
            ...
            <div className="relative">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">{partnerOffer.businessName}</h3>
              <p className="text-violet-100 text-lg mb-4">{partnerOffer.offer}</p>
        ...
        <section className="mt-8 bg-gradient-to-br from-slate-800 to-slate-800/50 rounded-3xl p-8 border border-slate-700/50">
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              <p className="text-blue-400 text-sm font-medium mb-1">Your Real Estate Advisor</p>
              ...
              {agent.license && (
                <p className="text-slate-600 text-xs mt-3">License: {agent.license}</p>
              )}
            </div>
            ...
            {company.logo && (
              <div className="flex-shrink-0 hidden md:block">
                <img src={company.logo} alt={company.name} className="h-16 w-auto object-contain opacity-60" />
              </div>
            )}
          </div>
          ...
          <div className="mt-6 p-6 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-2xl border border-blue-500/20 text-center">
            <p className="text-white font-semibold mb-2">Ready to discuss this property?</p>
            <p className="text-slate-400 text-sm mb-4">
              I'm here to answer your questions and help you make informed decisions.
            </p>
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <a
                href={agent.email ? `mailto:${agent.email}?subject=Property%20Inquiry:%20${encodeURIComponent(address?.address || '')}` : "#"}
        ...
        <footer className="text-center pt-12 mt-12 border-t border-slate-800">
          ...
          <p className="text-slate-500 text-sm">
            Report prepared {new Date(report.savedAt).toLocaleDateString("en-US", {
              month: "long",
              day: "numeric",
              year: "numeric",
            })}
          ...
          {company.phone && (
            <p className="text-slate-500 text-sm mt-2">
              ...
              {company.email && <> · <Mail className="h-3 w-3 inline mx-1" /> {company.email}</>}
            </p>
          )}
          ...
          <p className="text-slate-700 text-xs mt-4 max-w-xl mx-auto">
            This report is for informational purposes only. Values are estimates based on market data. Consult with licensed professionals before making decisions.
          </p>
        </footer>
      </main>
    </div>
