  ...
  try {
    ...
    if (action === "processNext") {
      ...
      try {
        ...
        console.log(
          `[Valuation Queue] Processing ${nextItem.id} for ${nextItem.propertyAddress}`,
        );

        const valuationRes = await fetch(
          `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/property/valuation`,
          {
            method: "POST",
        ...
        const aiRes = await fetch(
          `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/property/ai-analysis`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        const saveRes = await fetch(
          `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/research-library`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        if (
          ...
          try {
            ...
            if (smsRes.ok) {
              console.log(
                `[Valuation Queue] SMS sent to ${nextItem.leadPhone}`,
              );
            } else {
              console.log(
                `[Valuation Queue] SMS send failed: ${smsRes.status}`,
              );
            }
          } catch (smsErr) {
        ...
        if (
          ...
          console.log(
            `[Valuation Queue] Email would be sent to ${nextItem.leadEmail}: ${shareableLink}`,
          );
        }

        console.log(
          `[Valuation Queue] Report delivered for ${nextItem.leadName}: ${shareableLink}`,
        );
