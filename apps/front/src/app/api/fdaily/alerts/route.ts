...
const SIGNALHOUSE_JWT =
  process.env.SIGNALHOUSE_JWT || process.env.SIGNALHOUSE_AUTH_TOKEN || "";
...
const CAMPAIGN_CONFIG: Record<string, CampaignConfig> = {
  MLS_LISTED: {
    enabled: true,
    smsTemplate:
      "Hi {{ownerName}}, noticed {{address}} just hit the market. If listing doesn't work out, I buy properties for cash with quick closing. Interested? -{{agentName}}",
  ...
  MLS_EXPIRED: {
    enabled: true,
    smsTemplate:
      "Hi {{ownerName}}, saw {{address}} listing expired. I know that's frustrating. I buy homes as-is, cash, fast close. Can we chat? -{{agentName}}",
  ...
  MLS_PRICE_DROP: {
    enabled: true,
    smsTemplate:
      "Hi {{ownerName}}, saw price drop on {{address}}. If you want to skip the listing hassle, I'll make a fair cash offer, close in 2 weeks. -{{agentName}}",
  ...
  AUCTION_SCHEDULED: {
    enabled: true,
    smsTemplate:
      "URGENT: {{ownerName}}, your property {{address}} has auction scheduled. I can pay cash TODAY to stop foreclosure. Call me: {{agentPhone}} -{{agentName}}",
  ...
  OCCUPANCY_CHANGE: {
    enabled: true,
    smsTemplate:
      "Hi {{ownerName}}, noticed you may have moved from {{address}}. If you're looking to sell, I buy properties as-is for cash. -{{agentName}}",
  ...
  ADDITIONAL_NOTICE: {
    enabled: true,
    smsTemplate:
      "{{ownerName}}, noticed new filing on {{address}}. I help homeowners avoid foreclosure by buying for cash. Can we talk today? -{{agentName}}",
  ...
  TAX_LIEN_ADDED: {
    enabled: true,
    smsTemplate:
      "Hi {{ownerName}}, saw tax lien on {{address}}. I can buy your property and handle the lien. Quick close, cash. Want to chat? -{{agentName}}",
...
async function sendSignalHouseSMS(
  to: string,
  message: string,
): Promise<boolean> {
  ...
  try {
    const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        ...
        from: {
          email: "alerts@homeowneradvisor.com",
  ...
  try {
    ...
    for (const change of changes as AlertPayload[]) {
      ...
      if (ownerPhone && config.smsTemplate) {
        ...
        const smsSent =
          (await sendSignalHouseSMS(ownerPhone, smsMessage)) ||
      ...
      if (ownerEmail && config.emailTemplate) {
        ...
        const emailSent = await sendSendGridEmail(
          ownerEmail,
          emailSubject,
          emailBody,
        );
        ...
        if (emailSent) {
          ...
          results.errors.push(
            `Email failed for ${change.leadId}: ${ownerEmail}`,
          );
        }
      }
    }
