...
async function executeAction(
  ...
  console.log(`[WorkflowEngine] Executing action: ${action}`, { value, leadId });

  try {
    switch (action) {
      case "add_tag": {
        if (!leadId || !value) return { success: false, error: "Missing leadId or tag value" };
        const [lead] = await db.select().from(leads).where(eq(leads.id, leadId)).limit(1);
        if (!lead) return { success: false, error: "Lead not found" };

        const currentTags = (lead.tags as string[]) || [];
        if (!currentTags.includes(value)) {
          await db.update(leads).set({ tags: [...currentTags, value] }).where(eq(leads.id, leadId));
        }
      ...
      case "remove_tag": {
        ...
        const currentTags = (lead.tags as string[]) || [];
        await db.update(leads).set({ tags: currentTags.filter(t => t !== value) }).where(eq(leads.id, leadId));
        return { success: true, result: { tag: value, removed: true } };
      }

      case "update_status": {
        if (!leadId || !value) return { success: false, error: "Missing leadId or status value" };
        await db.update(leads).set({ status: value }).where(eq(leads.id, leadId));
        return { success: true, result: { status: value } };
      }

      case "send_sms": {
        if (!leadId) return { success: false, error: "Missing leadId" };
        if (!isConfigured()) return { success: false, error: "SignalHouse not configured" };
        ...
        const message = value || `Hi ${lead.firstName || "there"}! Thanks for reaching out.`;
        const fromNumber = process.env.SIGNALHOUSE_FROM_NUMBER || "";

        const result = await sendSMS({ to: lead.phone, from: fromNumber, message });
      ...
      case "add_notes": {
        if (!leadId || !value) return { success: false, error: "Missing leadId or note value" };
        const [lead] = await db.select().from(leads).where(eq(leads.id, leadId)).limit(1);
        if (!lead) return { success: false, error: "Lead not found" };

        const currentNotes = (lead.notes as string) || "";
        const newNote = `[${new Date().toISOString()}] ${value}`;
        await db.update(leads).set({ notes: currentNotes + "\n" + newNote }).where(eq(leads.id, leadId));
...
async function executeWorkflows(
  trigger: string,
  context: { teamId: string; leadId?: string; data?: Record<string, unknown> }
): Promise<{ executed: number; results: Array<{ workflowId: string; success: boolean; steps: unknown[] }> }> {
  const matchingWorkflows = WORKFLOW_REGISTRY.filter(
    (wf) => wf.trigger === trigger && wf.active
  );

  console.log(`[WorkflowEngine] Found ${matchingWorkflows.length} workflows for trigger: ${trigger}`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[WorkflowEngine] Processing trigger: ${trigger}`, { teamId, leadId });
...
export async function GET() {
  return NextResponse.json({
    ...
    triggers: ["message.received", "lead.created", "lead.updated", "campaign.started"],
    actions: [
      "add_tag",
      "remove_tag",
      "update_status",
