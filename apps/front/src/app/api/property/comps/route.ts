...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (targetProperty) {
      ...
      searchParams.beds_min = String(
        Math.max(1, (targetProperty.bedrooms || 3) - 1),
      );
      searchParams.beds_max = String((targetProperty.bedrooms || 3) + 1);
      searchParams.sqft_min = String(
        Math.floor((targetProperty.squareFeet || 1500) * 0.8),
      );
      searchParams.sqft_max = String(
        Math.ceil((targetProperty.squareFeet || 1500) * 1.2),
      );
    } else if (lat && lng) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const searchCriteria = {
      ...
      sqft_min:
        criteria?.sqft_min || Math.floor((subject.squareFeet || 1500) * 0.8),
      sqft_max:
        criteria?.sqft_max || Math.ceil((subject.squareFeet || 1500) * 1.2),
      year_built_min:
        criteria?.year_built_min || (subject.yearBuilt || 1990) - 10,
      year_built_max:
        criteria?.year_built_max || (subject.yearBuilt || 1990) + 10,
