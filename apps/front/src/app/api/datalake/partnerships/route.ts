  ...
  try {
    ...
    if (sicCodes && Array.isArray(sicCodes)) {
      ...
      for (const catId of categories) {
        const cat =
          PARTNERSHIP_CATEGORIES[catId as keyof typeof PARTNERSHIP_CATEGORIES];
        if (cat) {
      ...
      return NextResponse.json(
        {
          ...
          availableCategories: Object.entries(PARTNERSHIP_CATEGORIES).map(
            ([id, cat]) => ({
              id,
              name: cat.name,
    ...
    if (businesses.length === 0) {
      return NextResponse.json({
        success: false,
        message:
          "No business data found in datalake. Upload USBizData business CSV first.",
    ...
    for (const biz of businesses) {
      ...
      if (zipCode) {
        const bizZip = String(
          biz["Zip Code"] || biz["zipCode"] || biz["Zip"] || "",
        ).trim();
      ...
      const record: BusinessRecord = {
        ...
        sicCode: bizSic,
        sicDescription: String(
          biz["SIC Description"] || biz["sicDescription"] || "",
        ),
      ...
      if (includeContact) {
        record.contactName = String(
          biz["Contact Name"] || biz["contactName"] || "",
        );
        ...
        record.employeeCount =
          parseInt(String(biz["Number of Employees"] || "0")) || undefined;
        record.annualRevenue =
          parseFloat(String(biz["Annual Revenue"] || "0")) || undefined;
      }
...
export async function GET() {
  const categories = Object.entries(PARTNERSHIP_CATEGORIES).map(
    ([id, cat]) => ({
      id,
      name: cat.name,
      description: cat.description,
