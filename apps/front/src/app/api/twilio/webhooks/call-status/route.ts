...
export async function POST(request: NextRequest) {
  try {
    ...
    if (callStatus === "completed") {
      ...
      console.log(`[Twilio] Call ${callSid} completed - Duration: ${callDuration}s`);

      // If answered by machine, could schedule a callback
      if (answeredBy === "machine_start" || answeredBy === "machine_end_beep") {
        console.log(`[Twilio] Machine detected - could schedule callback for lead ${leadId}`);
      }
    }

    if (callStatus === "no-answer" || callStatus === "busy" || callStatus === "failed") {
      console.log(`[Twilio] Call ${callSid} ${callStatus} - could schedule retry`);
    }
