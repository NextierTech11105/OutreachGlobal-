...
const API_KEYS = {
  ...
  anthropic: { env: "ANTHROPIC_API_KEY", name: "Anthropic Claude", category: "llm" },
  ...
  realestateapi: { env: "REALESTATE_API_KEY", altEnv: "REAL_ESTATE_API_KEY", name: "RealEstateAPI (Skip Trace)", category: "data" },
  apollo: { env: "APOLLO_IO_API_KEY", altEnv: "APOLLO_API_KEY", name: "Apollo.io", category: "data" },

  // Communication
  signalhouse: { env: "SIGNALHOUSE_API_KEY", name: "SignalHouse SMS", category: "sms" },
  twilio_sid: { env: "TWILIO_ACCOUNT_SID", name: "Twilio Account SID", category: "voice" },
  twilio_token: { env: "TWILIO_AUTH_TOKEN", name: "Twilio Auth Token", category: "voice" },
  sendgrid: { env: "SENDGRID_API_KEY", name: "SendGrid", category: "email" },

  // Storage - check both variants
  do_spaces_key: { env: "DO_SPACES_KEY", altEnv: "SPACES_KEY", name: "DO Spaces Key", category: "storage" },
  do_spaces_secret: { env: "DO_SPACES_SECRET", altEnv: "SPACES_SECRET", name: "DO Spaces Secret", category: "storage" },
...
export async function GET() {
  ...
  for (const [id, config] of Object.entries(API_KEYS)) {
    ...
    const isConfigured = !!process.env[config.env] || (altEnv ? !!process.env[altEnv] : false);
    const usedEnvVar = process.env[config.env] ? config.env : (altEnv && process.env[altEnv] ? altEnv : config.env);
  ...
  const grouped = status.reduce((acc, item) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const altEnv = (config as { altEnv?: string }).altEnv;
    const apiKey = process.env[config.env] || (altEnv ? process.env[altEnv] : undefined);
      ...
      switch (provider) {
        case "openai":
          ...
          testResult = {
            ...
            message: openaiRes.ok ? "OpenAI connected" : `OpenAI error: ${openaiRes.status}`,
          };
          break;
        ...
        case "anthropic":
          ...
          testResult = {
            ...
            message: apiKey.startsWith("sk-ant-") ? "Anthropic key format valid" : "Invalid key format",
          };
          break;

        case "apollo":
          const apolloRes = await fetch("https://api.apollo.io/api/v1/auth/health", {
            headers: { "x-api-key": apiKey },
          });
          testResult = {
            success: apolloRes.ok,
            message: apolloRes.ok ? "Apollo.io connected" : `Apollo error: ${apolloRes.status}`,
          };
          break;

        case "realestateapi":
          const reRes = await fetch("https://api.realestateapi.com/v1/AutoComplete", {
            method: "POST",
            headers: {
              "x-api-key": apiKey,
          ...
          testResult = {
            ...
            message: reRes.ok || reRes.status === 400 ? "RealEstateAPI connected" : `Error: ${reRes.status}`,
          };
          break;
        ...
        case "sendgrid":
          testResult = {
            ...
            message: apiKey.startsWith("SG.") ? "SendGrid key format valid" : "Invalid key format",
          };
          break;
