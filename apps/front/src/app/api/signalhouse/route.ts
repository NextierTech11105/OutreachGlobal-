...
const SIGNALHOUSE_BASE_URL = process.env.SIGNALHOUSE_BASE_URL || "https://api.signalhouse.io/v1";

// Check if SignalHouse is configured
function isConfigured(): boolean {
  return !!(SIGNALHOUSE_API_KEY || (SIGNALHOUSE_ACCOUNT_SID && SIGNALHOUSE_AUTH_TOKEN));
}
...
export async function GET(request: NextRequest) {
  ...
  if (!isConfigured()) {
    return NextResponse.json({
      configured: false,
      error: "SignalHouse API not configured. Set SIGNALHOUSE_API_KEY or SIGNALHOUSE_ACCOUNT_SID + SIGNALHOUSE_AUTH_TOKEN",
    }, { status: 503 });
  }
  ...
  try {
    if (action === "numbers") {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({ error: `SignalHouse error: ${err}` }, { status: response.status });
      }
  ...
  try {
    ...
    switch (action) {
      case "send_sms": {
        ...
        console.log(`[SignalHouse] Sending SMS to ${to}: ${message.substring(0, 50)}...`);
      ...
      case "provision_number": {
        ...
        console.log(`[SignalHouse] Provisioning number for ${campaignName} (area code: ${areaCode || "any"})`);

        // Search available numbers
        const searchResponse = await fetch(`${SIGNALHOUSE_BASE_URL}/AvailablePhoneNumbers?area_code=${areaCode || ""}&capabilities=${capabilities || "sms,mms"}`, {
          method: "GET",
          headers: getAuthHeaders(),
        });
        ...
        if (!searchResponse.ok) {
          ...
          return NextResponse.json({ error: `Failed to search numbers: ${err}` }, { status: searchResponse.status });
        }

        const available = await searchResponse.json();
        const numbers = available.available_phone_numbers || available.data || [];
        ...
        const purchaseResponse = await fetch(`${SIGNALHOUSE_BASE_URL}/IncomingPhoneNumbers`, {
          method: "POST",
          headers: getAuthHeaders(),
          body: JSON.stringify({
        ...
        if (!purchaseResponse.ok) {
          ...
          return NextResponse.json({ error: `Failed to provision number: ${err}` }, { status: purchaseResponse.status });
        }
      ...
      case "verify_phone": {
        ...
        const response = await fetch(`${SIGNALHOUSE_BASE_URL}/Lookups/${encodeURIComponent(phone)}?type=carrier`, {
          method: "GET",
          headers: getAuthHeaders(),
        });
        ...
        if (!response.ok) {
          const err = await response.text();
          return NextResponse.json({ error: `Lookup failed: ${err}` }, { status: response.status });
        }
          ...
          phones.slice(0, 100).map(async (phone: string) => {
            try {
              ...
              return {
                ...
                mobile: lookup.carrier?.type === "mobile" || lookup.line_type === "mobile",
              };
            } catch {
