...
async function enrichPropertyLead(propertyId: string): Promise<EnrichedContact | null> {
  try {
    ...
    if (!detailResponse.ok) {
      console.error(`[Prepare Contact] Property detail failed: ${detailResponse.status}`);
      return null;
    }
    ...
    const ownerFirstName = ownerInfo.owner1FirstName || property.owner1FirstName || property.ownerFirstName || "";
    const ownerLastName = ownerInfo.owner1LastName || property.owner1LastName || property.ownerLastName || "";
    const propertyAddress = address.address || address.street || address.label || "";
    const propertyCity = address.city || "";
    const propertyState = address.state || "";
    const propertyZip = address.zip || "";

    console.log(`[Prepare Contact] Owner: ${ownerFirstName} ${ownerLastName}, Address: ${propertyAddress}`);
    ...
    if (REALESTATE_API_KEY && (ownerFirstName || ownerLastName || propertyAddress)) {
      ...
      if (skipTraceResponse.ok) {
        ...
        if (result.phones && Array.isArray(result.phones)) {
          phones = result.phones.map((p: { number?: string; phoneNumber?: string; type?: string; score?: number }) => ({
        ...
        if (result.emails && Array.isArray(result.emails)) {
          emails = result.emails.map((e: { email?: string; address?: string; type?: string }) => ({
        ...
        console.log(`[Prepare Contact] Skip trace found ${phones.length} phones, ${emails.length} emails`);
      } else {
        console.warn(`[Prepare Contact] Skip trace failed: ${skipTraceResponse.status}`);
      }
    }
...
async function enrichB2BLead(companyId: string, personId?: string): Promise<EnrichedContact | null> {
  try {
    if (!APOLLO_API_KEY) {
    ...
    if (personId) {
      const personResponse = await fetch(`https://api.apollo.io/v1/people/${personId}`, {
        headers: {
          "Content-Type": "application/json",
      ...
      if (personResponse.ok) {
        ...
        return {
          ...
          phones: person.phone_numbers?.map((p: { number: string; type?: string }) => ({
            number: p.number,
            type: p.type || "work",
          })) || [],
    ...
    const companyResponse = await fetch(`https://api.apollo.io/v1/organizations/${companyId}`, {
      headers: {
        "Content-Type": "application/json",
        "X-Api-Key": APOLLO_API_KEY,
      },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === "property") {
      ...
      return NextResponse.json({ error: "type must be 'property' or 'b2b'" }, { status: 400 });
    }
    ...
    const bestPhone = enrichedData.phones.find(p =>
    ...
    return NextResponse.json({
      ...
      recommended: {
        ...
        displayName: enrichedData.fullName || enrichedData.companyName || "Unknown",
      },
      stats: {
        phonesFound: enrichedData.phones.length,
        emailsFound: enrichedData.emails.length,
        hasMobile: enrichedData.phones.some(p =>
          p.type?.toLowerCase() === "mobile" || p.type?.toLowerCase() === "cell"
        ),
      },
    });
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    endpoints: {
      property: hasRealEstateApi ? "PropertyDetail + SkipTrace v1" : "Not configured",
      b2b: hasApolloApi ? "Apollo People/Organizations" : "Not configured",
    },
    usage: "POST with { id, type: 'property' | 'b2b' } to enrich before contact",
  });
}
