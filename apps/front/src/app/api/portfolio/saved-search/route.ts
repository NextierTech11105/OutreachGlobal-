  ...
  try {
    ...
    if (!search_name || !search_query) {
      return NextResponse.json(
        { error: "search_name and search_query are required" },
        { status: 400 },
      );
    }

    console.log("Creating RealEstateAPI Saved Search:", {
      search_name,
      list_size,
    });
    ...
    if (!response.ok || data.statusCode !== 200) {
      return NextResponse.json(
        {
          error: data.statusMessage || "Failed to create saved search",
          details: data,
        },
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok || data.statusCode !== 200) {
      return NextResponse.json(
        {
          error: data.statusMessage || "Failed to retrieve saved search",
          details: data,
        },
        { status: response.status },
      );
    ...
    const grouped = {
      ...
      unchanged: results.filter(
        (r: any) => r.changeType === null || r.changeType === "unchanged",
      ),
    };
