  ...
  try {
    ...
    for (const stat of stateStats) {
      ...
      territory.avgScore = Math.round(
        (territory.avgScore * (territory.states.length - 1) + (Number(stat.avgScore) || 0)) /
    ...
    // Summary
    const totalCompanies = territories.reduce((sum, t) => sum + t.companyCount, 0);
    const totalSmsReady = territories.reduce((sum, t) => sum + t.smsReadyCount, 0);
