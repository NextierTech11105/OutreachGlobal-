...
export async function POST(request: NextRequest) {
  try {
    ...
    if (query && query.trim()) {
      ...
      conditions.push(
        or(
          ilike(businesses.companyName, q),
          ilike(businesses.ownerName, q),
          ilike(businesses.city, q),
        ),
      );
    }
    ...
    if (states && states.length > 0) {
      conditions.push(
        or(...states.map((s: string) => eq(businesses.state, s.toUpperCase()))),
      );
    }

    if (sicCodes && sicCodes.length > 0) {
      conditions.push(
        or(...sicCodes.map((sic: string) => eq(businesses.sicCode, sic))),
      );
    }
      ...
      conditions.push(
        or(
          ...industries.map((ind: string) =>
            ilike(businesses.sicDescription, "%" + ind + "%"),
          ),
        ),
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: "Search failed", companies: [], total: 0 },
      { status: 500 },
    );
  }
}
