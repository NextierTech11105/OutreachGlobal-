...
export interface USBizDataDatabase {
  ...
  price: number; // USD
  category: 'blue_collar' | 'professional' | 'healthcare' | 'real_estate' | 'retail' | 'technology' | 'financial' | 'other';
  tags: string[];
...
export const USBIZDATA_COLUMNS = {
  ...
  employees: ['Number of Employees', 'number_of_employees', 'Employees', 'EMPLOYEES'],
  revenue: ['Annual Revenue', 'annual_revenue', 'Revenue', 'REVENUE'],
  sicCode: ['SIC Code', 'sic_code', 'SICCode', 'SIC'],
  sicDescription: ['SIC Description', 'sic_description', 'SICDescription', 'SIC DESC'],
} as const;
...
export function getDatabase(idOrSlug: string): USBizDataDatabase | undefined {
  return USBIZDATA_REGISTRY.find(
    db => db.id === idOrSlug || db.slug === idOrSlug || db.sicCode === idOrSlug
  );
}
...
export function getDatabasesByCategory(category: USBizDataDatabase['category']): USBizDataDatabase[] {
...
export function searchDatabases(query: string): USBizDataDatabase[] {
  ...
  return USBIZDATA_REGISTRY.filter(db => 
...
export function getCategoryStats(): Record<string, { count: number; records: number }> {
...
export function luciQuery(query: LuciQuery): LuciQueryResult {
  ...
  const estimatedRecords = matches.reduce((sum, db) => sum + db.totalRecords, 0);
  
  // Generate recommendation
  let recommendation = '';
  if (matches.length === 0) {
    recommendation = 'No matching databases found. Try broadening your search or uploading a custom CSV.';
...
console.log('[USBizData Registry] Loaded', USBIZDATA_REGISTRY.length, 'databases with', getTotalRecordsAvailable().toLocaleString(), 'total records');
