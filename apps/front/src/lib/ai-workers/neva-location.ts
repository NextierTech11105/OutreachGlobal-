...
export async function validateBusinessAddress(
  ...
  if (
    geocoded.components.stateCode &&
    geocoded.components.stateCode !== state
  ) {
    issues.push({
      type: "state_mismatch",
  ...
  if (
    zip &&
    geocoded.components.postalCode &&
    !geocoded.components.postalCode.startsWith(zip.substring(0, 5))
  ) {
    issues.push({
      type: "zip_mismatch",
  ...
  if (
    geocoded.accuracy === "approximate" ||
    geocoded.accuracy === "interpolated"
  ) {
    issues.push({
      type: "partial_match",
        ...
        try {
          ...
          return {
            ...
            validation: {
              ...
              issues: [
                {
                  type: "address_not_found" as const,
